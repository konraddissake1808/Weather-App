...
export async function GET(req: NextRequest) {
	...
	const locationsLat = []
	const locationsLon = []
	locationsLat.push(parseFloat(lat));
	locationsLon.push(parseFloat(lon));
	...
	for (let i=0; i < data.length; i++) {
		locationsLat.push(data[i].latitude);
		locationsLon.push(data[i].longitude);
	}

	console.log(`${locationsLat}, ${locationsLon}`);

	const params = {
		"latitude": locationsLat,
		"longitude": locationsLon,
